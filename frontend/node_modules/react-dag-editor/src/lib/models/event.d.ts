...
export declare enum GraphPortEvent {
    Click = "[Port]Click",
...
export declare enum GraphScrollBarEvent {
    ScrollStart = "[ScrollBar]ScrollStart",
    Scroll = "[ScrollBar]Scroll",
    ScrollEnd = "[ScrollBar]ScrollEnd"
}
export declare enum GraphMinimapEvent {
    PanStart = "[Minimap]PanStart",
    Pan = "[Minimap]Pan",
    PanEnd = "[Minimap]PanEnd",
    Click = "[Minimap]Click"
}
export declare enum GraphContextMenuEvent {
    Open = "[ContextMenu]Open",
    Close = "[ContextMenu]Close"
}
export interface ICanvasCommonEvent extends IEventBase {
    type: Exclude<GraphCanvasEvent, GraphCanvasEvent.ViewportResize | GraphCanvasEvent.Navigate | GraphCanvasEvent.VirtualizationRecalculated | GraphCanvasEvent.Zoom | GraphCanvasEvent.Drag | GraphCanvasEvent.MouseWheelScroll | GraphCanvasEvent.Pinch | GraphCanvasEvent.SelectStart | GraphCanvasEvent.SelectMove | GraphCanvasEvent.UpdateNodeSelectionBySelectBox | GraphCanvasEvent.Copy | GraphCanvasEvent.Paste | GraphCanvasEvent.Undo | GraphCanvasEvent.Redo | GraphCanvasEvent.Delete | GraphCanvasEvent.KeyDown | GraphCanvasEvent.KeyUp | GraphCanvasEvent.DraggingNodeFromItemPanel | GraphCanvasEvent.DraggingNodeFromItemPanelEnd | GraphCanvasEvent.ScrollIntoView | GraphCanvasEvent.ResetUndoStack | GraphCanvasEvent.ResetViewport | GraphCanvasEvent.ZoomTo | GraphCanvasEvent.ZoomToFit | GraphCanvasEvent.SetData | GraphCanvasEvent.UpdateData | GraphCanvasEvent.Pan | GraphCanvasEvent.UpdateSettings>;
}
...
export interface INodeCommonEvent<NodeData = unknown, PortData = unknown> extends IEventBase {
    type: Exclude<GraphNodeEvent, GraphNodeEvent.Resizing | GraphNodeEvent.DragStart | GraphNodeEvent.Drag | GraphNodeEvent.DragEnd | GraphNodeEvent.Click | GraphNodeEvent.SelectAll | GraphNodeEvent.Centralize | GraphNodeEvent.Locate | GraphNodeEvent.Add | GraphNodeEvent.ContextMenu | GraphNodeEvent.Select>;
    node: NodeModel<NodeData, PortData>;
}
...
export interface INodeClickEvent<NodeData = unknown, PortData = unknown> extends IEventBase<MouseEvent | React.MouseEvent> {
...
export type INodeEvent<NodeData = unknown, PortData = unknown> = INodeCommonEvent<NodeData, PortData> | INodeResizeEvent<NodeData, PortData> | INodeDragStartEvent<NodeData, PortData> | INodeDragEvent<NodeData, PortData> | INodeDragEndEvent<NodeData, PortData> | INodeSimpleEvent | INodeCentralizeEvent | INodeLocateEvent | INodeContextMenuEvent<NodeData, PortData> | INodeClickEvent<NodeData, PortData> | INodeAddEvent<NodeData, PortData> | INodeSelectEvent;
export interface IEdgeCommonEvent<T = unknown> extends IEventBase {
    type: Exclude<GraphEdgeEvent, GraphEdgeEvent.ConnectStart | GraphEdgeEvent.ConnectMove | GraphEdgeEvent.ConnectEnd | GraphEdgeEvent.ConnectNavigate | GraphEdgeEvent.Add>;
    edge: EdgeModel<T>;
}
...
export type IEdgeEvent<T> = IEdgeCommonEvent<T> | IEdgeConnectStartEvent | IEdgeConnectMoveEvent | IEdgeConnectEndEvent | IEdgeConnectNavigateEvent | IEdgeAddEvent<T>;
...
export interface ICanvasUpdateSettingsEvent<NodeData = unknown, EdgeData = unknown, PortData = unknown> extends Partial<IGraphSettings<NodeData, EdgeData, PortData>> {
    type: GraphCanvasEvent.UpdateSettings;
}
export type IContextMenuEvent = IContextMenuOpenEvent | IContextMenuCloseEvent;
export type IEvent<NodeData = unknown, EdgeData = unknown, PortData = unknown> = (ICanvasEvent<NodeData, EdgeData, PortData> | INodeEvent<NodeData, PortData> | IEdgeEvent<EdgeData> | IPortEvent<NodeData, PortData> | IScrollBarEvent | IMinimapEvent | IContextMenuEvent | ICanvasUpdateSettingsEvent<NodeData, EdgeData, PortData>) & {
    intercepted?: boolean;
};
export {};
